...
class BatchConverter(object):
    ...
    def __call__(self, raw_batch: Sequence[Tuple[str, str]], max_len=0):
        ...
        for i, (label, seq_str) in enumerate(raw_batch):
            ...
            # )
            parts = seq_str.split('<pad>')
            indices = []
            for part in parts:
                if part:
                    indices.extend([self.alphabet.get_idx(s) for s in part])
                indices.append(self.alphabet.get_idx('<pad>'))

            seq = torch.tensor(indices[:-1], dtype=torch.int64)

            tokens[i, int(self.alphabet.prepend_bos):max_len + int(self.alphabet.prepend_bos)] = seq
            if self.alphabet.append_eos:
                tokens[
                    i, max_len + int(self.alphabet.prepend_bos)
