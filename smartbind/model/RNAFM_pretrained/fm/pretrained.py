...
def load_model_and_alphabet_core(model_data, regression_data=None, theme="protein", device="cpu"):
    ...
    model = model_type(
        Namespace(**model_args), alphabet, device
    )
...
def rna_fm_t12(model_location=None, device="cpu"):
    if model_location is not None and os.path.exists(model_location):
        ...
        return load_rnafm_model_and_alphabet_hub("rna_fm_t12", theme="rna", device=device)


def load_rnafm_model_and_alphabet_hub(model_name, theme="rna", device="cpu"):
    ...
    return load_model_and_alphabet_core(model_data, regression_data, theme, device=device)
