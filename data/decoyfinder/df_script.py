### Reference: https://github.com/hongli-ma/RNAsmol/blob/main/scripts/modified_decoyfinder.py
...
def find_decoys(active_set,
                ...
                mind = 100,
                unique = False,
                max_db_mols = 1000000000,
                output_file = None): # output_file: path to save results incrementally
                        ...
                        if is_decoy(ligand, db_mol, HBA_t, HBD_t, ClogP_t, MW_t, RB_t, tanimoto_t):
                            ...
                            if not too_similar_to_existing:
                                ...
                                if ligands_dict[ligand] >= mind: # If decoy set complete for the active ligand
                                    ...
                                    # Save incrementally when one ligand is complete
                                    if output_file:
                                        print(f'Completed decoys for {ligand.name}. Saving to {output_file}...')
                                        _, temp_smiles_dict = make_decoy_dict(decoy_set)
                                        with open(output_file, 'w') as f:
                                            json.dump(temp_smiles_dict, f, indent=2)
                                if unique:
                                    break
            except Exception as e:
...
if __name__ == '__main__':
    # read active ligands
    active = read_active_ligands(
        filename="data/hariboss_actives.smi",
        delimiter=" ",
        titleLine=True,     
    )

    # construct decoy candidate library generator
    db_gen = parse_database(
        # db_dir="data/decoy_db/",
        db_dir="data/zinc20/library_prepared_smi/",
        delimiter=" ",
        titleLine=True,
        do_shuffle=True
    )

    # run search with incremental saving
    output_file = "decoys_smiles_zinc20_ml_top200.json"
    smiles_decoy_dict = find_decoys(
        active_set=active,
        db_generator=db_gen,
        mind=200,  
        # max_db_mols=50000000,  
        output_file=output_file  
    )

    # save final results
    print(f'Search complete. Final results saved to {output_file}')
    with open(output_file, "w") as f:
        json.dump(smiles_decoy_dict, f, indent=2)
